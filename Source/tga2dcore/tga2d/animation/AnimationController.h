...
namespace Tga2D
{
	...
	public:
		...
		bool Add3DAnimFromFile(const std::wstring& fileName)
		{
			...
			Assimp::Importer* tempImporter = new Assimp::Importer;
			const aiScene* tempAiScene = tempImporter->ReadFile(string_cast<std::string>(fileName), aiProcessPreset_TargetRealtime_Quality | aiProcess_ConvertToLeftHanded);
			// If the import failed, report it
			if (tempAiScene)
			{
				if (tempAiScene->mNumAnimations > 0)
				{
					logInfo(tempImporter->GetErrorString());
					m_GlobalInverseTransform = tempAiScene->mRootNode->mTransformation;
					m_GlobalInverseTransform.Inverse();
