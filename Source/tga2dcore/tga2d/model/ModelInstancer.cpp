...
#include "tga2d/shaders/InstancedModelShader.h"

ID3D11Buffer* Tga2D::ModelInstancer::GetInstanceBuffer() const
{
	return myInstanceBuffer;
}

void Tga2D::ModelInstancer::Init(std::shared_ptr<Model> aModel)
{
	myModel = aModel;
...
bool Tga2D::ModelInstancer::AddInstance(const Transform& aTransform)
{
...
bool Tga2D::ModelInstancer::RemoveInstance(unsigned int anIdx)
{
...
void Tga2D::ModelInstancer::RebuildInstances()
{
	if(isDirty)
	{
		isDirty = false;
...
void Tga2D::ModelInstancer::Render(InstancedModelShader& aShader) const
{
