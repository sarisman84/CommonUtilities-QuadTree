...
void CreatePreviewForAsset(std::wstring filePath)
{
    ...
    if (hasMesh) 
    {
        ...
        if (animatedModel->IsValid()) //If the model has an animation
        {
            ...
            std::vector<AnimationImportDefinition> animationPaths;
            animationPaths.push_back({filePath, true});
...
void Go(void)
{
    ...
    createParameters.myActivateDebugSystems = Tga2D::DebugFeature::None;/*Tga2D::DebugFeature::Fps |
...

    {
        ...
        float rgb[3];
        rgb[0] = 100.0f/255.0f; //set startup color to dark gray
        rgb[1] = 100.0f/255.0f;
        rgb[2] = 100.0f/255.0f;
        ...
        lightDirections[0] = dLight.GetTransform().GetRotation().X;
        lightDirections[1] = dLight.GetTransform().GetRotation().Y;
        lightDirections[2] = dLight.GetTransform().GetRotation().Z;
...
#pragma region ImGUI stuff
#ifndef _RETAIL
            Tga2D::Engine::GetInstance()->GetMyIMGUIInterface().PreFrame();
            if (ImGui::Begin("Settings", &isOpen, 0))
            {
                ...
                ImGui::SliderFloat3("Directional Light", lightDirections, 0, 360);
                if (ImGui::Button("Reset Light Rotation", { 150,30 })) {
                    lightDirections[0] = 270;
                    lightDirections[1] = 0;
                    lightDirections[2] = 0;
                }
...
#pragma endregion
            ...
            if (GlobalRenderData.myAnimatedModel) //If the current model has an animation
            {
                ...
                {
                    GlobalRenderData.myAnimatedModel->PlayAnimation(GlobalRenderData.myAnimatedModel->GetAnimationCount() - 1);
...
#pragma region Camera
            ...
            if (!Tga2D::Engine::GetInstance()->BeginFrame(Color(rgb[0], rgb[1], rgb[2])))
            {   
                ImGui::End();
                ImGui::EndFrame();
                break;
...
#pragma endregion
            ...
            Tga2D::Engine::GetInstance()->GetMyIMGUIInterface().Render();
